...
- **Python / Node without sudo:**
  - Python: sqlcipher3 (zlib license, https://github.com/coleifer/sqlcipher3). **CORRECTED 2026-09-25 — the earlier claim in this memo was stale.** `sqlcipher3` 0.6.2 (uploaded 2026-01-07) ships **78 wheels covering all three platforms**: macOS (universal2, x86_64, arm64), Linux (manylinux_2_28 x86_64/aarch64/i686, musllinux_1_2 x86_64/aarch64) and Windows (win32, win_amd64, win_arm64). Wheels were downloaded and unpacked to confirm they are real: win_amd64 is a single 5.9 MB `_sqlite3.cp312-win_amd64.pyd`, manylinux is a 21 MB `.so`, both statically bundling SQLCipher 4.x (confirmed by `cipher_version` and `sqlcipher_codec_ctx_init_kdf_salt` symbols) with no external libsqlcipher. **No build toolchain is needed on any platform.** The Linux-x86_64-only limitation applied to the superseded `sqlcipher3-binary` package name, which never shipped a macOS or Windows wheel across its 13 versions. (`sqlcipher3-wheels`, a laggykiller fork, is also cross-platform but pins SQLCipher 3.x; 0.6.2's METADATA credits laggykiller as co-author — the efforts merged.)
...
Yes for the embedded-database family. SQLCipher is a library, not a service; nothing about it requires root. The practical friction is *packaging*, not privilege, and **the Python packaging friction is now resolved**: sqlcipher3 0.6.2 ships wheels for Linux, macOS (incl. arm64) and Windows (corrected 2026-09-25 — see §3.1), so nothing must be built or vendored. On Node, better-sqlite3-multiple-ciphers (MIT) also ships prebuilds inside its npm tarball (darwin-arm64/x64, linux-arm64/x64, linuxmusl-arm64/x64, win32-arm64/x64) — note it has no GitHub release assets, which makes a naive check look empty. By contrast @journeyapps/sqlcipher (BSD-3) runs `node-gyp rebuild` on install, so every user needs a full C++ toolchain; avoid it. FUSE vaults are sudo-free on Linux *if* /dev/fuse and the FUSE package are present, but on macOS they pull in macFUSE (a kernel extension historically installed with admin rights) — macFUSE on macOS 26 can use an FSKit backend to run filesystems in user space, and **FUSE-T** (https://github.com/macos-fuse-t/fuse-t) deliberately replaces macFUSE with a user-space NFSv4/SMB/FSKit server "instead of a kernel extension", precisely because "it's getting harder and harder to load kernel extensions" on macOS. Either way it is extra machinery. **Embedded DB wins on both privilege and portability.**
...
**Recommendation (with the trade-off stated).** Build the store on **SQLCipher (BSD-3-Clause)** — or **SQLite3MultipleCiphers (MIT)** if you want an all-MIT stack — holding content, transcripts, the FTS5 index, and sqlite-vec embeddings in one encrypted file, with attachments encrypted as AEAD blobs under the same key hierarchy. **Trade-off:** you accept that a single-writer embedded DB is the concurrency model (and you must pin a SQLite version >= 3.42 for sqlite-vec's `MATCH`, preferring the `k =` form over `LIMIT`). The former build/vendor step for macOS/arm64 is no longer needed — sqlcipher3 0.6.2 ships wheels for all three platforms (corrected 2026-09-25). In exchange you get the only architecture in this survey that protects the *index and embeddings* by construction, needs no root, no mount lifecycle, and no kernel extension, and is proven in production by Signal and Joplin. **Keep DuckDB (MIT) on WATCH** as the eventual analytical/timeline engine — its WAL and temp-file encryption are exactly right — and re-evaluate it once its NIST gap (#20162) closes and its vector index leaves "experimental".
...
4. ~~**sqlcipher3-binary macOS/arm64 wheels**~~ — **CLOSED 2026-09-25.** The premise was stale: sqlcipher3 0.6.2 ships wheels for Linux, macOS (universal2/x86_64/arm64) and Windows, verified by unpacking them. Only the superseded `sqlcipher3-binary` name was Linux-only. Residual unknown: no direct evidence found of sqlcipher3 0.6.2 being tested *together with* sqlite-vec, though the bundled SQLite 3.51.1 satisfies sqlite-vec's >= 3.42 requirement.
